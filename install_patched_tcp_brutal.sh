...
TCP_BRUTAL_DEFAULT_RATE="10485760"  # bytes/s，默认 10 MiB/s
TCP_BRUTAL_RATE="${TCP_BRUTAL_RATE:-$TCP_BRUTAL_DEFAULT_RATE}"  # bytes/s，可在运行脚本前通过环境变量覆盖，也可安装时交互设置
DOMAINS=()
IPV4_ADDRS=()
IPV6_ADDRS=()
...
# 校验 brutal-nginx 速率参数。单位：bytes/s。默认 10485760，即 10 MiB/s。
validate_tcp_brutal_rate() {
    if ! [[ "$TCP_BRUTAL_RATE" =~ ^[0-9]+$ ]] || (( TCP_BRUTAL_RATE < 1 )); then
        print_warning "TCP_BRUTAL_RATE=$TCP_BRUTAL_RATE 非法，已回退为 ${TCP_BRUTAL_DEFAULT_RATE} bytes/s"
        TCP_BRUTAL_RATE="$TCP_BRUTAL_DEFAULT_RATE"
    fi
}

# 交互设置 brutal-nginx 速率参数。输入单位为 bytes/s；留空使用默认 10 MiB/s。
select_tcp_brutal_rate() {
    local input_rate=""

    echo ""
    echo -e "${GREEN}请选择 tcp_brutal_rate:${NC}"
    echo "单位: bytes/s"
    echo "默认: ${TCP_BRUTAL_DEFAULT_RATE} bytes/s (10 MiB/s)"
    echo "示例: 10485760=10 MiB/s, 52428800=50 MiB/s, 104857600=100 MiB/s"
    read -p "请输入 tcp_brutal_rate [默认 ${TCP_BRUTAL_DEFAULT_RATE}]: " input_rate

    if [[ -n "$input_rate" ]]; then
        TCP_BRUTAL_RATE="$input_rate"
    fi

    validate_tcp_brutal_rate
    print_info "tcp_brutal_rate 已设置为: ${TCP_BRUTAL_RATE} bytes/s"
}
...
# 写入 tcp_brutal 指令。
# 仅写入真正承载 Xray XHTTP 后端流量的 location，避免在 server、静态站点、ACME、80 跳转中重复/无效生成。
...
full_install() {
    ...
    # 设置 tcp_brutal_rate，后续 Nginx 最终配置生成时使用。
    select_tcp_brutal_rate
